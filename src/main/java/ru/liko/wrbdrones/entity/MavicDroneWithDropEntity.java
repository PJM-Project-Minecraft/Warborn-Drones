...
public class MavicDroneWithDropEntity extends AddonDroneEntity {
    ...
    private static final Set<String> ALLOWED_ATTACHMENTS = Set.of(
            ...
            "warbornexplosives:rgo",
            "warbornexplosives:rgn");

    private static final Set<String> GRENADE_ITEMS = Set.of(
            "superbwarfare:rgo_grenade",
            "warbornexplosives:rgo",
            "warbornexplosives:rgn");
    ...
    public @NotNull InteractionResult interact(@NotNull Player player, @NotNull InteractionHand hand) {
        ...
        // Специальная обработка для гранат - проверяем ограничение на сервере ДО вызова
        // super
        if (!stack.isEmpty() && !player.isCrouching() && GRENADE_ITEMS.contains(itemId)
                && ALLOWED_ATTACHMENTS.contains(itemId)) {
            ...
            if (!this.level().isClientSide()) {
                ...
                if (ammoAccessor != null) {
                    ...
                    if (currentAmmo >= MAX_GRENADES) {
                        @SuppressWarnings("null")
                        @NotNull
                        Component message = Component.translatable("tips.wrbdrones.drone.max_grenades_reached")
        ...
        // После добавления гранаты ограничиваем AMMO и MAX_AMMO до 2 (только на
        // сервере)
        if (!stack.isEmpty() && GRENADE_ITEMS.contains(itemId) && !this.level().isClientSide()) {
            var ammoAccessor = getAmmoAccessor();
    ...
    public void baseTick() {
        ...
        if (!this.level().isClientSide()) {
            ...
            if (ammoAccessor != null && maxAmmoAccessor != null) {
                ...
                try {
                    java.lang.reflect.Field currentItemField = com.atsuishio.superbwarfare.entity.vehicle.DroneEntity.class
                    ...
                    if (currentItem != null && !currentItem.isEmpty()) {
                        String currentItemId = getItemId(currentItem);
                        if (GRENADE_ITEMS.contains(currentItemId)) {
                            int currentAmmo = this.getEntityData().get(ammoAccessor);
                            int maxAmmo = this.getEntityData().get(maxAmmoAccessor);
