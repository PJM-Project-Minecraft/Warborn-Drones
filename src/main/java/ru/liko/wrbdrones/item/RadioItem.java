...
public class RadioItem extends Item {
            ...
            if (tag.getBoolean(TAG_LINKED)) {
                ...
                if (!shahedUuidStr.isEmpty()) {
                    ...
                    if (shahed != null && !shahed.isLaunched()) {
                        PacketDistributor.sendToPlayer(serverPlayer, new OpenRadioScreenPacket(
                                shahed.getId(),
    ...
    public static InteractionResult onInteractWithShahed(Player player, Shahed136Entity shahed, ItemStack radioStack) {
        ...
        CompoundTag tag = NBTTool.getTag(radioStack);
        ...
        if (player.isShiftKeyDown()) {
            ...
            shahed.setLinkedRadioUUID(tag.hasUUID("RadioUUID")
                    ? tag.getUUID("RadioUUID").toString()
                    : UUID.randomUUID().toString());

            // Если RadioUUID не было, сохраним его
            if (!tag.hasUUID("RadioUUID")) {
                tag.putUUID("RadioUUID", UUID.fromString(shahed.getLinkedRadioUUID()));
            }
        ...
        NBTTool.saveTag(radioStack, tag);
    ...
    public void appendHoverText(@NotNull ItemStack stack, @Nullable Item.TooltipContext context,
        ...
        // super.appendHoverText(stack, context, tooltip, flag); // RadioItem extends
        // Item
    ...
    public boolean isFoil(@NotNull ItemStack stack) {
        CompoundTag tag = NBTTool.getTag(stack);
        return tag.getBoolean(TAG_LINKED);
    }
}
