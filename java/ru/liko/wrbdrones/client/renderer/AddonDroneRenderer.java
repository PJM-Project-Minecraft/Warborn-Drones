...
public class AddonDroneRenderer<T extends AddonDroneEntity> extends GeoEntityRenderer<T> {
    ...
    public void renderRecursively(PoseStack poseStack, T animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource,
        ...
        // Управление видимостью гранат - применяется ТОЛЬКО для костей granade1 и granade2
        if ("granade1".equals(name) || "granade2".equals(name)) {
            ...
            if (animatable instanceof MavicDroneWithDropEntity) {
                ...
                // Для всех остальных дронов (включая MavicDroneNoDropEntity и FpvDroneEntity) скрываем гранаты
                bone.setHidden(true);
            }
        }
        ...
        if ("camera".equals(name)) {
            ...
            // Используем отрицательное значение, так как в GeckoLib поворот по X инвертирован
    ...
    private int getAmmoCount(T entity) {
        try {
            ...
            if (ammoAccessor != null) {
                ...
                try {
                    Class<?> vehicleEntityClass = com.atsuishio.superbwarfare.entity.vehicle.DroneEntity.class.getSuperclass();
                    if (vehicleEntityClass != null) {
                        ...
                        net.minecraft.network.syncher.EntityDataAccessor<Integer> ammoAccessorDirect = 
                            (net.minecraft.network.syncher.EntityDataAccessor<Integer>) ammoField.get(null);
    ...
    /**
     * Проверяет, прикреплена ли граната (rgo_grenade) к дрону
     * Использует DISPLAY_ENTITY для проверки на клиенте (синхронизируется через EntityData)
     */
    private boolean isGrenadeAttached(T entity) {
        try {
            // Используем DISPLAY_ENTITY для проверки типа боеприпаса (синхронизируется между клиентом и сервером)
            ...
            if (displayEntityAccessor != null) {
                ...
                if (displayEntity != null && !displayEntity.isEmpty()) {
                    // Проверяем, что это граната RGO
                    if (displayEntity.contains("rgo_grenade") || displayEntity.equals("superbwarfare:rgo_grenade")) {
                        return true;
                    }
                }
            } else {
                ...
                if (!entity.level().isClientSide()) {
                    ...
                    if (CURRENT_ITEM_FIELD != null) {
                        com.atsuishio.superbwarfare.entity.vehicle.DroneEntity droneEntity = 
                            (com.atsuishio.superbwarfare.entity.vehicle.DroneEntity) entity;
                        net.minecraft.world.item.ItemStack currentItem = 
                            (net.minecraft.world.item.ItemStack) CURRENT_ITEM_FIELD.get(droneEntity);
                        
                        if (currentItem != null && !currentItem.isEmpty()) {
                            var key = net.minecraftforge.registries.ForgeRegistries.ITEMS.getKey(currentItem.getItem());
                            if (key != null && "superbwarfare:rgo_grenade".equals(key.toString())) {
                                return true;
                            }
                        }
                    }
    ...
    private static synchronized void initCurrentItemField() {
        if (currentItemFieldInitialized) return;
        
        try {
            CURRENT_ITEM_FIELD = com.atsuishio.superbwarfare.entity.vehicle.DroneEntity.class.getDeclaredField("currentItem");
    ...
    private static synchronized void initDisplayEntityField() {
        if (displayEntityFieldInitialized) return;
        
        try {
            DISPLAY_ENTITY_FIELD = com.atsuishio.superbwarfare.entity.vehicle.DroneEntity.class.getDeclaredField("DISPLAY_ENTITY");
    ...
    private boolean hasAmmo(T entity) {
        ...
        if (DISPLAY_ENTITY_FIELD != null) {
            try {
                net.minecraft.network.syncher.EntityDataAccessor<String> displayEntityAccessor = 
                    (net.minecraft.network.syncher.EntityDataAccessor<String>) DISPLAY_ENTITY_FIELD.get(null);
        ...
        if (CURRENT_ITEM_FIELD != null) {
            try {
                ...
                net.minecraft.world.item.ItemStack currentItem = (net.minecraft.world.item.ItemStack) CURRENT_ITEM_FIELD.get(droneEntity);
    ...
    private static boolean isRotorBone(String name) {
        return "wingFL".equals(name) || "wingFR".equals(name) || "wingBL".equals(name) || "wingBR".equals(name)
                || "rotor_fl".equals(name) || "rotor_fr".equals(name) || "rotor_bl".equals(name) || "rotor_br".equals(name)
    ...
    private float getControllerPitch(T animatable, float partialTick) {
        ...
        if (mainHand.getItem() == com.atsuishio.superbwarfare.init.ModItems.MONITOR.get()) {
            String linkedDrone = mainHand.getOrCreateTag().getString("LinkedDrone");
            boolean isUsing = mainHand.getOrCreateTag().getBoolean("Using");
            boolean isLinked = mainHand.getOrCreateTag().getBoolean("Linked");
        ...
        for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
            ...
            if (stack.getItem() == com.atsuishio.superbwarfare.init.ModItems.MONITOR.get()) {
                String linkedDrone = stack.getOrCreateTag().getString("LinkedDrone");
                boolean isUsing = stack.getOrCreateTag().getBoolean("Using");
                boolean isLinked = stack.getOrCreateTag().getBoolean("Linked");
