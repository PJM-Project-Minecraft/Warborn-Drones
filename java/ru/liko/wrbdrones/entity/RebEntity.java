...
public class RebEntity extends Entity implements GeoEntity {
    private static final EntityDataAccessor<Boolean> IS_ENABLED = SynchedEntityData.defineId(RebEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Float> HEALTH = SynchedEntityData.defineId(RebEntity.class, EntityDataSerializers.FLOAT);
    private static final float MAX_HEALTH = 20.0F; // Максимальное здоровье РЭБ
    ...
    /**
     * Force render РЭБ на любом расстоянии для всех игроков.
     */
    @Override
        ...
        if (player.isShiftKeyDown()) {
            ...
            if (!this.level().isClientSide()) {
                if (!player.getAbilities().instabuild) {
                    ItemHandlerHelper.giveItemToPlayer(player, new ItemStack(ModItems.REB.get()));
                }
                this.discard();
            }
            ...
            if (!this.level().isClientSide() && player instanceof ServerPlayer serverPlayer) {
                ...
                if (newState) {
                    ...
                    serverPlayer.displayClientMessage(Component.translatable("wrbdrones.reb.enabled"), true);
                } else {
                    this.level().playSound(null, this.getX(), this.getY(), this.getZ(), 
                            ModSounds.REB_TOGGLE_OFF.get(), SoundSource.BLOCKS, 1.0f, 1.0f);
                    serverPlayer.displayClientMessage(Component.translatable("wrbdrones.reb.disabled"), true);
                }
            }
            return InteractionResult.sidedSuccess(this.level().isClientSide());
        }
